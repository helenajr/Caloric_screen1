...
#If HS == "Hypo" MS is converted to NA, so it won't get included in the model
data <- mutate(data,
               MS = ifelse((HS == "Hypo"), NA, MS))

#Create the logistic regression model and get the predicted values out
library(rms)
# Switch gold_call to binary variable (required for model functions).
data <- mutate(data,
               gc_binary = if_else(gold_call == 'Pos', 1, 0))

# Make a data set (d) containing only columns used in the model
d <- select(data, Order, MS, gc_binary) 

# Use the datadist function to collect distribution parameters that are then used by the model
dd <- datadist(d); 
options(datadist= 'dd')

# Create the model
f <- lrm(gc_binary ~ rcs(MS, 3) * Order, data=d)
f

data <- mutate(data,
               prediction = predict(f, type  = "fitted"))

#ROC analysis of both hot and cold together
roc_results <- rocit(score = data$prediction, class = data$gold_call, negref = "Neg")

summary(roc_results)
ciAUC(roc_results, nboot = 1000)
...
gold_sum <- drop_na(data)
gold_sum <- summary(as.factor(gold_sum$gold_call), na.omit = T)
...
data3 <- data3 %>% mutate_at(2:10, round, 2)
data3 <- mutate(data3,
                Cutoff = round(data3$Cutoff, 3),
                WarmMS = seq(1, 4),
                ColdMS = seq(1, 4))
data3 <- relocate(data3, WarmMS, ColdMS)

write.table(data3, file = "summary2.txt", sep = ",", quote = FALSE, row.names = F)

########Convert cutoffs to MS values for hot and cold - by basically creating a lookup table?!
#### Horrible code alert #####
x_MS <- seq(0, 25, 0.01)
...
cat("Cutoff required for 99% specificity for cool test ", cut_cool)
...
cat("Cutoff required for 90% specificity for warm test ", cut_warm)
...
croc_results <- rocit(score = c_data$prediction, class = c_data$gold_call, negref = "Neg")
wroc_results <- rocit(score = w_data$prediction, class = w_data$gold_call, negref = "Neg")

summary(croc_results)
ciAUC(croc_results, nboot = 1000)
summary(wroc_results)
ciAUC(wroc_results, nboot = 1000)
...
c_data_tibble <- tibble(
                      Cutoff=croc_results$Cutoff, 
...
data3 <- relocate(data3, WarmMS)

write.table(data3, file = "summary_warm.txt", sep = ",", quote = FALSE, row.names = F)

###Manually fill in WarmMS by running some of the horrible code section above.....
...
plot_combo <- filter(combo, Cutoff <=0.5)
